...
namespace Urd.Test
{
    public class TestNavigationBoomerangManager
    {
        ...
        private BoomerangInfoModel _boomerangInfoModel;
        ...
        public void SetUp()
        {
            ...
            _boomerangInfoModel = new BoomerangInfoModel();
        }

        [UnityTest]
        public IEnumerator NavigationBoomerangManager_Open_Success()
        {
            _navigationService.Open(_boomerangInfoModel, OnOpenNavigable);
        ...
        [UnityTest]
        public IEnumerator NavigationBoomerangManager_IsOpen_Success()
        {
            _navigationService.Open(_boomerangInfoModel, OnOpenNavigable);

            yield return new WaitUntil(() => _onOpenCallback);
            bool isOpen = _navigationService.IsOpen(_boomerangInfoModel);
            
            Assert.That(_onOpenCallbackValue && isOpen, Is.True);
        }

        [UnityTest]
        public IEnumerator NavigationBoomerangManager_Close_Success()
        {
            _navigationService.Open(_boomerangInfoModel, OnOpenNavigable);
            yield return new WaitUntil(() => _onOpenCallback);
            
            _navigationService.Close(_boomerangInfoModel, OnCloseNavigable);
